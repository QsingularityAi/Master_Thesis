...
def data_preprocessing(input_data, target_label1, target_label2):
    
    df=pd.DataFrame(data=input_data[0:,0:],
            index=[i for i in range(input_data.shape[0])],
           columns=['f'+str(i) for i in range(input_data.shape[1])])
    df2 = pd.DataFrame(target_label1)
    df3 = pd.DataFrame(target_label2)


    input_cols = list(df.columns)
    target_cols1 = list(df2.columns)
    target_cols2 = list(df3.columns)

    inputs_df = df[input_cols].copy()
    target_df1 = df2[target_cols1].copy()
    target_df2 = df3[target_cols2].copy()
