...
#gamma2 = .99
#step_size2 = 100
optimizer2 = torch.optim.Adam(MTL2.parameters(), lr=0.0024559181335557663, weight_decay= 3.3113878820970685e-08)
...
def main():
    ...
    for it in range(epochs):
        ...
        with torch.no_grad():
            ...
            l1D = loss_func2(test1_predict, Y1_test.view(-1,1))
            l2D = loss_func2(test2_predict, Y2_test.view(-1,1))
            l3D = loss_func2(test3_predict, Y1_test.view(-1,1))
            l4D = loss_func2(test4_predict, Y2_test.view(-1,1))
            test_loss = l1D+l2D
            cost1ts_2.append(test_loss)
            cost11ts.append(l1D)
            cost12ts.append(l2D)
            cost3ts_2.append(l3D)
            cost4ts_2.append(l4D)
            costts_2.append(test_loss+l3D+l4D)    
